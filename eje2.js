...
let cantidadTotalCanciones = banda.albums.reduce( (totalCanciones, discoObj)=>{
    return totalCanciones+ discoObj.songs.length
}, 0 );
let duracionTotal= banda.albums.reduce((totalDuracion, discoObj)=>{
    return totalDuracion + discoObj.duration
},0);
let promedioDuracion = duracionTotal/cantidadTotalCanciones;
